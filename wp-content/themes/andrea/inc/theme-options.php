...
function andrea_theme_options_add_page() {
	add_theme_page( __( 'Theme Options' ), __( 'Theme Options' ), 'edit_theme_options', 'theme_options', 'andrea_theme_options_do_page' );
}
			...
			<table class="form-table form-wrap">
				...
				<tr valign="top"><th scope="row"><?php _e( 'Choose a layout option:' ); ?></th>
					<td>
						...
						?>
								<label class="description"><input type="radio" name="andrea_theme_options[layout_choice]" value="<?php echo esc_attr( $option['value'] ); ?>" <?php echo $checked; ?> /> <strong><?php echo $option['label']; ?></strong> &mdash; <span class="description"><?php echo $option['description']; ?></span></label>
								<?php
							}
						?>
						</fieldset>
