...
if ( function_exists('register_sidebar') ) {
    register_sidebar(array(
		'name' => 'Left',
		'description' => __( 'An empty left header widget area will show your blog tagline.', 'depo-square' ),
        'before_widget' => '',
        'after_widget' => '',
...
register_sidebar(array(
	'name' => 'Center',
	'description' => __( 'An empty center header widget area will show your latest status message.', 'depo-square' ),
    'before_widget' => '',
    'after_widget' => '',
    'before_title' => '<h3 class="widgettitle">',
    'after_title' => '</h3>',
...
register_sidebar(array(
	'name' => 'Right',
	'description' => __( 'An empty right header widget area will show interesting photos from Flickr.', 'depo-square' ),
    'before_widget' => '',
    'after_widget' => '',
    'before_title' => '<h3 class="widgettitle">',
    'after_title' => '</h3>',
...
if( is_admin() ) {
	...
	function deposquare_add_page() {
		add_theme_page(__('Theme Options'), __('Theme Options'), 'edit_theme_options', 'theme_options', 'deposquare_do_page');
	}
				...
				<td>
					<table border="0" cellspacing="5" cellpadding="5">
						...
						<?php foreach( $depo_post_types as $key => $option ) {
							if($key != 'post') {
							?>
						<tr><td><?php echo $option['proper_name']; ?></td><td>
...
function old_fetch_rss_summary( $a, $b ) { return fetch_rss( $a, $b ); }
...
class Widget_DePoFlickr extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		$flickr_rss_url = esc_url_raw( $instance['flickr_rss_url'] );
		...
		if(  !$photos = @wp_cache_get('widget-depo-square-flickr') ) {
			$flickr_photos = old_fetch_rss_summary( $flickr_rss_url, array( 'link', 'title', 'media:thumbnail', 'description' ) );
			if( is_array( $flickr_photos->items ) ) {
				$photos = '';
				$items = array_slice( $flickr_photos->items, 0, $items );
				while( list( $key, $photo ) = each( $items ) ) {
					$href = esc_url( $photo['link'] );
					$itemtitle = esc_attr( $photo['title'] );
					if ( isset( $photo['media:thumbnail'] ) ) {
						$src = str_replace( "_s.jpg", "_t.jpg", esc_url( $photo['media:thumbnail']['url'] ) );
				...
				$flickr_home = esc_url( $flickr_photos->channel[ 'link' ] );
				$flickr_more_title = esc_attr( $flickr_photos->channel[ 'title' ] );
