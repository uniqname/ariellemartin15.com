...
function slideshow_image_attachment_fields_to_edit($form_fields, $post) {
	...
	$form_fields["add-to-slideshow"] = array(
		"label" => __("Add to Slideshow"),
		"input" => "html",
		"html"  => "<input type='checkbox' value='1'
								name='attachments[{$post->ID}][add-to-slideshow]'
								id='attachments[{$post->ID}][add-to-slideshow]' />",
		"helps" => __("Check if you want this image to be added to the main site slideshow"),
	);
   return $form_fields;
}
// now attach our function to the hook
add_filter("attachment_fields_to_edit", "slideshow_image_attachment_fields_to_edit", null, 2);

function slideshow_image_attachment_fields_to_save($post, $attachment) {
	// $attachment part of the form $_POST ($_POST[attachments][postID])
        // $post['post_type'] == 'attachment'
	if( isset($attachment['add-to-slideshow']) ){
		// update_post_meta(postID, meta_key, meta_value);
		update_post_meta($post['ID'], '_add-to-slideshow', $attachment['add-to-slideshow']);
	}
	return $post;
}
// now attach our function to the hook.
add_filter("attachment_fields_to_save", "slideshow_image_attachment_fields_to_save", null , 2);
...
if ( function_exists( 'register_nav_menus' ) ) {
	register_nav_menus(
		array(
		  'manual_menu' => 'Manually curated menu for main navigation'
		)
	);
}

/* Enque necessary scripts */
function enqueue_js_Scripts() {
	if ( !is_admin() ) { // instruction to only load if it is not the admin area
	   // register your script location, dependencies and version
	   wp_deregister_script('jquery');
		 wp_register_script('jquery', 'http://ajax.googleapis.com/ajax/libs/jquery/1.6/jquery.min.js', false, '1.4.4');
	   wp_register_script('modernizr', get_bloginfo('template_directory') . '/js/modernizr-1.7.min.js','','', false );
	   wp_register_script('lettering', get_bloginfo('template_directory') . '/js/jquery.lettering-0.6.1.min.js','','', true );
	   wp_register_script('jqTools', get_bloginfo('template_directory') . '/js/jquery.tools.min.js','','', true);
	   // enqueue the script
