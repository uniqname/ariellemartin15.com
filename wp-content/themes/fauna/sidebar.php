<div id="sidebar">
	...
	<div class="box">
		...
		<?php if ( have_posts() ) : while ( have_posts() ) : the_post(); ?>
			...
			<p>"<a href="<?php the_permalink() ?>" title="<?php echo esc_attr( sprintf( __( 'Permanent Link: %s' ), the_title_attribute( 'echo=0' ) ) ); ?>"><?php the_title(); ?></a>" was written <?php the_time('F jS, Y') ?>
		...
		<?php if ( comments_open() && pings_open() ) {
			...
			<p>&darr; <a href="#comments">Read comments</a>, <a href="#respond">respond</a> or follow responses via <?php post_comments_feed_link( __( 'RSS' ) ); ?>.</p>
		...
		<?php } elseif ( comments_open() && !pings_open() ) {
			...
			<p>There are <?php comments_number('leave a response','one response','% responses'); ?>.</p>
			<p>&darr; <a href="#comments">Jump to comments</a> or follow responses via <?php post_comments_feed_link( __( 'RSS' )); ?>.</p>
			<?php if (function_exists('show_manual_subscription_form')) { show_manual_subscription_form(); }; ?>
		<?php } elseif ( !comments_open() && !pings_open() ) {
	...
	<?php /* Dynamic Sidebar */ if ( !is_category() && !is_single() && !is_page() && !is_search() && !is_author() ) { ?>
	<div class="box">
