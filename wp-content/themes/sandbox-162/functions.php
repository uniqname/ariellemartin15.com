...
register_nav_menus( array(
	'main' => __( 'Main Menu Area', 'twentyten' ),
) );
...
function sandbox_gallery($string, $attr) {
	...
	foreach ( $attachments as $id => $attachment ) {
		...
		$img_src = $img_src[0];
		$img_alt = esc_attr( $attachment->post_excerpt );
		if ( $img_alt == null )
			$img_alt = esc_attr( $attachment->post_title );
...
?>
		<?php echo $before_widget; ?>
			<?php echo $before_title . $title . $after_title; ?>
			<ul>
				<li><a href="<?php bloginfo('rss2_url') ?>" title="<?php echo esc_attr( get_bloginfo( 'name', 'display' ) ); ?> <?php _e( 'Posts RSS feed', 'sandbox' ); ?>" rel="alternate" type="application/rss+xml"><?php _e( 'All posts', 'sandbox' ) ?></a></li>
				<li><a href="<?php bloginfo('comments_rss2_url') ?>" title="<?php echo esc_attr( get_bloginfo( 'name', 'display' ) ); ?> <?php _e( 'Comments RSS feed', 'sandbox' ); ?>" rel="alternate" type="application/rss+xml"><?php _e( 'All comments', 'sandbox' ) ?></a></li>
			</ul>
		<?php echo $after_widget; ?>
<?php
}
