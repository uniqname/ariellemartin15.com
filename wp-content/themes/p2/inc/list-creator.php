 ...
 *
 * Parses unordered and ordered lists on save (storing the HTML in the database): if you switch themes, the list markup is preserved
 * Parses task lists on display: if you switch themese, you no longer have a handler for checkbox state toggles, so just show ASCII x's and o's.
 *
 * The checked state for each task list item is initially stored as an x (checked) or o (unchecked) in the post/comment text.
 * When a task list item is updated from the blog (via the HTML form), the checked state is stored in post/comment meta.
 * When the post/comment is edited, the checked states stored in post/comment meta in copied to the post/comment text (updating x's and o's) and deleted)
 *
 * This way, the order of the task list items can be changed while editing the post/comment without breaking the data stored in post/comment meta.
 */
