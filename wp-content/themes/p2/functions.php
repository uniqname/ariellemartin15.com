...
function iphone_js() {
	if ( strstr( $_SERVER['HTTP_USER_AGENT'], 'iPhone' ) or isset($_GET['iphone']) && $_GET['iphone'] ) { ?>
		<script type="text/javascript">
			jQuery( function( $ ) {
				$( 'a#mobile-post-button' ).click( function( e ) {
					e.preventDefault();
					if ( $( 'div#postbox' ).is( ':visible' ) )
						$( 'div#postbox' ).slideUp( 'fast' );
					else
						$( 'div#postbox' ).slideDown( 'fast' );
				});
			} );
		</script>
	<?php }
...
/**
 * Add post format support, back-compatible with previous used categories.
 *
 * @since P2 1.3.4
