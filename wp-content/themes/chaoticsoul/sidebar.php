	...
	<?php if ( !function_exists('dynamic_sidebar') || !dynamic_sidebar() ) : ?>
		...
		<?php } else { ?>
			...
			<h3><?php _e('About This Page'); ?></h3>
			<p><?php printf(__('You are currently browsing the %s weblog archives for the day %s.', 'chaoticsoul'), '<a href="'.get_bloginfo('url').'/">'.get_bloginfo('name').'</a>', get_the_time(get_option('date_format')));?></p>

			<?php /* If this is a monthly archive */ } elseif (is_month()) { ?>
			<h3><?php _e('About This Page'); ?></h3>
			<p><?php printf(__('You are currently browsing the %s weblog archives for %s.', 'chaoticsoul'), '<a href="'.get_bloginfo('url').'/">'.get_bloginfo('name').'</a>', get_the_time('F, Y'));?></p>

      <?php /* If this is a yearly archive */ } elseif (is_year()) { ?>
			<h3><?php _e('About This Page'); ?></h3>
			<p><?php printf(__('You are currently browsing the %s weblog archives for the year %s.', 'chaoticsoul'), '<a href="'.get_bloginfo('url').'/">'.get_bloginfo('name').'</a>', get_the_time('Y'));?></p>

		 <?php /* If this is a search */ } elseif (is_search()) { ?>
			<h3><?php _e('About This Page'); ?></h3>
			<p><?php printf(__('You have searched the %s weblog archives for %s. If you are unable to find anything in these search results, you can try one of these links.', 'chaoticsoul'), '<a href="'.get_bloginfo('url').'/">'.get_bloginfo('name').'</a>', '<strong>'.get_search_query().'</strong>');?></p>

			<?php /* Paged archive */ } elseif (is_paged()) { ?>
			<h3><?php _e('About This Page'); ?></h3>
			<p><?php printf(__('You are currently browsing the %s weblog archives.', 'chaoticsoul'), '<a href="'.get_bloginfo('url').'/">'.get_bloginfo('name').'</a>');?></p>
