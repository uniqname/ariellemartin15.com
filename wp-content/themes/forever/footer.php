		...
		if ( is_home() ) {
			...
			while ( $recent_formatted->have_posts() ) : $recent_formatted->the_post(); ?>
				<div id="latest-message" class="format-<?php echo get_post_format(); ?>">
					...
					<p class="permalink"><a href="<?php the_permalink(); ?>" title="<?php printf( esc_attr__( 'Permalink to %s', 'forever' ), the_title_attribute( 'echo=0' ) ); ?>" rel="bookmark"><?php the_title(); ?></a></p>
				</div><!-- #latest-message -->
			<?php endwhile;
		} ?>
