...
function brand_new_day_admin_enqueue_scripts( $hook_suffix ) {
	wp_enqueue_style( 'bnd-theme-options', get_template_directory_uri() . '/inc/theme-options.css', false, '20110428' );
}
...
function brand_new_day_theme_options_init() {
	...
	add_settings_field( 'theme_style', __( 'Theme Style', 'brand-new-day' ), 'brand_new_day_settings_field_theme_style', 'theme_options', 'general' );
	add_settings_field( 'sidebar_options', __( 'Sidebar Options', 'brand-new-day' ), 'brand_new_day_settings_field_sidebar_options', 'theme_options', 'general' );
	add_settings_field( 'remove_search', __( 'Remove Search', 'brand-new-day' ), 'brand_new_day_settings_field_remove_search', 'theme_options', 'general' );
	add_settings_field( 'simple_blog', __( 'Simple Blog Mode', 'brand-new-day' ), 'brand_new_day_settings_field_simple_blog', 'theme_options', 'general' );
}
...
function brand_new_day_theme_options_add_page() {
	$theme_page = add_theme_page(
		__( 'Theme Options', 'brand-new-day' ),   // Name of page
		__( 'Theme Options', 'brand-new-day' ),   // Label in menu
...
function brand_new_day_theme_styles() {
	$theme_style_options = array(
		'daylight' => array(
			...
			'thumbnail' => get_template_directory_uri() . '/inc/images/daylight.png',
		),
		'nightlight' => array(
			'value' => 'nightlight',
			'label' => __( 'Nightlight', 'brand-new-day' ),
			'thumbnail' => get_template_directory_uri() . '/inc/images/nightlight.png',
		),
		'winterlight' => array(
			'value' => 'winterlight',
			'label' => __( 'Winterlight', 'brand-new-day' ),
			'thumbnail' => get_template_directory_uri() . '/inc/images/winterlight.png',
		),
		'autumnlight' => array(
			'value' => 'autumnlight',
			'label' => __( 'Autumnlight', 'brand-new-day' ),
			'thumbnail' => get_template_directory_uri() . '/inc/images/autumnlight.png',
		),
	);
...
function brand_new_day_theme_options_render_page() {
	...
	<div class="wrap">
		<?php screen_icon(); ?>
		<h2><?php printf( __( '%s Theme Options', 'brand-new-day' ), get_current_theme() ); ?></h2>
		<?php settings_errors(); ?>
