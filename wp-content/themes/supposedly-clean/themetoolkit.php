...
//			$this->pluginification();
		...
		function add_menu() {
			global $wp_version;
			if ( $wp_version >= 2 ) {
				$level = 'edit_theme_options';
			} else {
				$level = 9;
			}
			add_theme_page(__('Theme Options', 'supposedly-clean'), __('Theme Options', 'supposedly-clean'), 'edit_theme_options', basename($this->file), array(&$this,'admin_menu'));
		...
		function store_options($array) {
			...
			if (update_option('theme-'.$this->infos['theme_shortname'].'-options',$array)) {
				return __('Options successfully stored', 'supposedly-clean');
			} else {
				return __('Could not save options !', 'supposedly-clean');
			}
		}
...
//			return "Theme options added in database (1 entry in table '". $wpdb->options ."')";
		...
		function admin_menu () {
			...
			// 	echo '<p>' . __('Please note that this theme is currently <strong>not activated</strong> on your site as the default theme.', 'supposedly-clean') . '</p>';
			// }

			$cache_settings = '';
			$check = $this->read_options();

			echo "<div class='wrap'><h2>" . __('Layout Options', 'supposedly-clean') . "</h2>";
			echo '<p>' . __('This theme allows you to configure some variables to suit your blog, which are:', 'supposedly-clean') . '</p>
			...
			echo '</table>
			<p class="submit"><input type="submit" value="' . __('Update Layout &raquo;', 'supposedly-clean') . '" /></p>
			</form></div>';
		}
