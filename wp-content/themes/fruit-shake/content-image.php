...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	<?php if ( $attachments and is_array( $attachments ) ) : ?>
		<?php $first_attachment = array_shift( $attachments ); ?>
		<a href="<?php the_permalink(); ?>" title="<?php echo esc_attr( sprintf( __( 'Permalink to %s', 'fruit-shake' ), the_title_attribute( 'echo=0' ) ) ); ?>" rel="bookmark"><img class="entry-image" src="<?php echo esc_url( wp_get_attachment_url( $first_attachment->ID ) ); ?>" alt="<?php echo esc_attr( strip_tags( $first_attachment->post_title ) ); ?>" /></a>
	<?php endif; ?>
