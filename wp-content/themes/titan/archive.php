...
<?php get_header(); ?>
	<?php if ( have_posts() ) : ?>
		<?php /* If this is a category archive */ if (is_category()) { ?>
			...
			<?php /* If this is an author archive */ } elseif ( is_author() ) { $author_id = get_queried_object_id(); ?>
			<h1 class="pagetitle"><?php printf( __( 'Posts by %s', 'titan' ), esc_html( get_the_author_meta( 'display_name', $author_id ) ) ); ?></h1>
	...
	<?php while ( have_posts() ) : the_post(); ?>
		<div class="entries">
			<ul>
				<li><span><a href="<?php the_permalink(); ?>" rel="bookmark" title="Permanent Link to <?php the_title_attribute(); ?>"><?php printf( __( '%s on', 'titan' ), get_the_title() ); ?></a> <?php the_time( get_option( 'date_format' ) ); ?></span></li>
			</ul>
		</div><!--end entries-->
